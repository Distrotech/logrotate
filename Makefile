...
install:
	[ -d $(DESTDIR)$(PREFIX)$(BINDIR) ] || mkdir -p $(DESTDIR)$(PREFIX)$(BINDIR)
	[ -d $(DESTDIR)$(PREFIX)$(MANDIR) ] || mkdir -p $(DESTDIR)$(PREFIX)$(MANDIR)
	[ -d $(DESTDIR)$(PREFIX)$(MANDIR)/man8 ] || mkdir -p $(DESTDIR)$(PREFIX)$(MANDIR)/man8
	[ -d $(DESTDIR)$(PREFIX)$(MANDIR)/man5 ] || mkdir -p $(DESTDIR)$(PREFIX)$(MANDIR)/man5

	if [ "$(OS_NAME)" = HP-UX ]; then \
	$(INSTALL) $(PROG) $(DESTDIR)$(PREFIX)$(BINDIR) 0755 bin bin; \
	$(INSTALL) $(MAN) $(DESTDIR)$(PREFIX)$(MANDIR)/man`echo $(MAN) | sed "s/.*\.//"` 0644 bin bin; \
	$(INSTALL) $(MAN5) $(DESTDIR)$(PREFIX)$(MANDIR)/man`echo $(MAN5) | sed "s/.*\.//"` 0644 bin bin; \
	...
	$(INSTALL) -m 755 $(PROG) $(DESTDIR)$(PREFIX)$(BINDIR); \
	$(INSTALL) -m 644 $(MAN) $(DESTDIR)$(PREFIX)$(MANDIR)/man`echo $(MAN) | sed "s/.*\.//"`/$(MAN); \
	$(INSTALL) -m 644 $(MAN5) $(DESTDIR)$(PREFIX)$(MANDIR)/man`echo $(MAN5) | sed "s/.*\.//"`/$(MAN5); \
	fi; fi
